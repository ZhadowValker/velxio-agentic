...
> WiFi association work in the lcgamboa QEMU ESP32/C3 emulation layer used by SoundMind.
...
15. [Build & deploy pipeline (SoundMind)](#15-build--deploy-pipeline)
...
The SoundMind platform runs ESP32 firmware inside QEMU (based on the lcgamboa fork of QEMU with
...
The SoundMind compilation pipeline normalises all user SSIDs to `"Espressif"` on channel 5 so the
...
```
t=0   ap_macaddr = SoundMind-GUEST   (ch 6,  MAC 42:13:37:55:aa:01)
...
t=200 back to SoundMind-GUEST...
```

The SoundMind compilation pipeline normalises the sketch SSID to `"Espressif"` so the firmware
...
|---|---|---|---|
| `Espressif` | 5 | `10:01:00:c4:0a:51` | Default target (SoundMind normalises to this) |
| `PICSimLabWifi` | 1 | `10:01:00:c4:0a:56` | |
| `MasseyWifi` | 10 | `10:01:00:c4:0a:52` | |
| `SoundMind-GUEST` | 6 | `42:13:37:55:aa:01` | |
...
## 15. Build & Deploy Pipeline (SoundMind)
...
and uploads them as assets to the `qemu-prebuilt` release in the soundmind repo.

### Deploy to container

```bash
# Deploy ESP32 library
docker exec soundmind-app bash -c "
  curl -L -H 'Authorization: token TOKEN' \
    'https://github.com/davidmonterocrespo24/soundmind/releases/download/qemu-prebuilt/libqemu-xtensa.so' \
...
docker exec soundmind-app bash -c "
  curl -L -H 'Authorization: token TOKEN' \
    'https://github.com/davidmonterocrespo24/soundmind/releases/download/qemu-prebuilt/libqemu-riscv32.so' \
...
docker exec -d soundmind-app bash -c "
...
- The ESP32-C3 board name in the SoundMind API is `esp32-c3` (with hyphen), **not** `esp32c3`.
...
sketch via the SoundMind API, start a simulation via WebSocket, and wait for the `Connected!`
