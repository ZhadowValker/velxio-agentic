 ...
 *
 * Converts between Wokwi's diagram.json format and SoundMind's internal
...
export interface SoundMindComponent {
  id: string;
  metadataId: string;
  x: number;
  y: number;
...
// Wokwi board type → SoundMind boardType
...
// SoundMind boardType → Wokwi type
...
// SoundMind boardType → default Wokwi part id
...
export async function exportToWokwiZip(
  ...
  zip.file(
    'wokwi-project.txt',
    `Exported from SoundMind\n\nSimulate this project on https://soundmind.dev\n`,
  );
  ...
  a.href = url;
  a.download = `${(projectName || 'soundmind-project').replace(/[^a-z0-9_-]/gi, '-')}.zip`;
...
export async function importFromWokwiZip(file: File): Promise<ImportResult> {
  ...
  // SoundMind internal component ID for the board element (must match DOM element id)
  ...
  const soundmindBoardId = VELXIO_BOARD_ID[boardType] ?? 'arduino-uno';
  ...
  // Convert non-board parts to SoundMind components.
  // Apply the same offset so components keep their relative position to the board.
  const components: SoundMindComponent[] = diagram.parts
  ...
  // Convert connections to SoundMind wires
  const wires: Wire[] = diagram.connections.map((conn, i) => {
    ...
    // Remap board part id → SoundMind internal board id
    const startId = startCompRaw === boardId ? soundmindBoardId : startCompRaw;
    const endId = endCompRaw === boardId ? soundmindBoardId : endCompRaw;
